...
- 行級渲染：上下兩句交替顯示，行首可提早顯示（預設 0.7s）
- 顏色群組：提供 A/B 群組管理頁，行級可選群組套用樣式
- 字幕樣式：字型/字級/描邊/陰影/字距/粗體/斜體可調
- 樣式預覽：唱前/唱後即時預覽，三層描邊示意
- 字型管理：匯入、重新掃描，日文字型優先顯示
...
目前進度：Phase 1.6（簡單預覽播放器）完成，Phase 2.3（字幕樣式編輯 UI）已完成顏色群組、樣式預覽、粗體/斜體、日文字型過濾等基礎功能。
